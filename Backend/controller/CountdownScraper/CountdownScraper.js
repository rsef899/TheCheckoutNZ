const express = require('express');
const axios = require('axios');
const fs = require('fs');
const app = express();

/* Setting initial request settings */

const target = 'browse';
let inStockProductsOnly = 'false';
let size = 120;
let page = 1;


let initUrl = new URL("https://www.countdown.co.nz/api/v1/products");
let url = initUrl;


let config = {
  method: 'get',
  maxBodyLength: Infinity,
  url: url,
  headers: { 
    'X-Requested-With': 'XMLHttpRequest', 
    'Cookie': '_abck=0AB975DA4B4FF835BD680D9C18ACA5AE~-1~~-1~-1~-1; ak_bmsc=8D18B256EF138BC38636F47626F8F039~000000000000000000000000000000~; bm_sz=E252B3A59ABDFCF976CC1B33CFB58E0C~~4604729~4404280; ASP.NET_SessionId=djccygbts2reansoogqybggn; aga=2c0c8125e3dde5a8ba08d8abc71345d2; agaCORS=2c0c8125e3dde5a8ba08d8abc71345d2; akavpau_vpwww=1688113804~id=69bcb3a26764edec7ffbdcd2dfae9754; cw-arjshtsw=v3bd8f5adcf124a6d9b9a8536c9fc15bbvtqhhplqh; cw-laie=d106d3a7f2a1412c9dee73de55292e59; dtid=2:oRLrBEa/Ve6EKSHvnd6aChrzrOi7oe5VIGVXslBmhI3qExmz+Zi0wMcpnvQf5e+gtKfjTyjXiZSq4Nu4PqBs+LjHpkglzHwBL9jiwHXhmCA3N+iNGDr3dQSgoTGu0jwAtUw='
  }
};
/**
 * getCategories() is used to collect the names of all the categories on the countdown website.
 * @returns array of names of the categories in Countdown
 */

async function getCategories() {
    
let catConfig = {
    method: 'get',
    maxBodyLength: Infinity,
    url: 'https://www.countdown.co.nz/api/v1/products?target=search&search=*',
    headers: { 
      'X-Requested-With': 'XMLHttpRequest', 
      'Cookie': '_abck=0AB975DA4B4FF835BD680D9C18ACA5AE~-1~~-1~-1~-1; ak_bmsc=8D18B256EF138BC38636F47626F8F039~000000000000000000000000000000~; bm_sz=E252B3A59ABDFCF976CC1B33CFB58E0C~~4604729~4404280; ASP.NET_SessionId=djccygbts2reansoogqybggn; aga=2c0c8125e3dde5a8ba08d8abc71345d2; agaCORS=2c0c8125e3dde5a8ba08d8abc71345d2; akavpau_vpwww=1688113672~id=69f4dc8f2bca768757528e47504d6d7d; cw-arjshtsw=v3bd8f5adcf124a6d9b9a8536c9fc15bbvtqhhplqh; cw-laie=d106d3a7f2a1412c9dee73de55292e59; dtid=2:oRLrBEa/Ve6EKSHvnd6aChrzrOi7oe5VIGVXslBmhI3qExmz+Zi0wMcpnvQf5e+gtKfjTyjXiZSq4Nu4PqBs+LjHpkglzHwBL9jiwHXhmCA3N+iNGDr3dQSgoTGu0jwAtUw='
    }
  };

    try {
        let response = await axios.request(catConfig);
        categories = JSON.parse(JSON.stringify(response.data));
        const namesArray = categories.dasFacets.map(item => item.name);
        
        return namesArray;
       
    
    } catch (error) {
        console.error(`ERRORCat: ${error}`)
    }

}

/**
 * getItems is used to grab all the items on a specific page from a given category, can only return 120 items
 * @param {Name of the category} category 
 * @param {Page number we are trying to search} page 
 * @returns 
 */
async function getItems(category,page) {

    // Setting initial parameters for the API call
    url.searchParams.set("dasFilter","Department;;"+category+";false");
    url.searchParams.set("target",target);
    url.searchParams.set("inStockProductsOnly",inStockProductsOnly);
    url.searchParams.set("size",size);
    url.searchParams.set("page",page);

    try {
        // Delay of 150 ms to avoid nuke
        await new Promise(r => setTimeout(r,150));

        let response = await axios.request(config)

        // Formating the data to the form we want to keep
        jObj = JSON.parse(JSON.stringify(response.data));
        jProducts = jObj["products"];
        totalItems = jProducts["totalItems"];
        
        
        // Filtering out and keeping only the 'Products' from the API call
        jProducts = jProducts.items.filter((items)=> items.type === "Product");
        
        // Return all the items from the call as well as the totalItems in the entire category (not the total from the call)
        console.log(jProducts.length);
        return [jProducts.flat(),totalItems];
   
    
    } catch (error) {
        console.error(`ERROR: ${error}`)
    }
}


async function main() {
    // Grab the names of all the categories on the countdown website.
    const categories = await getCategories();
    let listItems = [];
    // Finding all items by category (then by pages in each category)
        for (category of categories) {
        const categoryItems = [];
        categoryName = category;
        // Renaming all categories to the name required for requests (& becomes - and removal of capital letters + spaces)
        console.log(`Checking category: ${categoryName}`);
        categoryName = categoryName.replace(/\s+/g, '');
        categoryName = categoryName.replace("&","-");
        categoryName = categoryName.toLowerCase();

        // Initial (first page) run
        page = 1;
        // Flag is used as a way to tell if the response we get back is empty or not
        let flag = true;

        // Grab the items and total number or items required using getItems function
        const [initItems,totalReq] = await getItems(categoryName,page);

        totalItems = initItems.length;

        // Push the items we got from the first run to the list of items from the category
        categoryItems.push(initItems);
        
        // Run for the rest of the pages in the category
        while ((totalItems <= totalReq) && (flag)) {
            try { // Handle successful result
                page++; 
                let result = await getItems(categoryName,page);
                let items = result[0];
                
                // If not empty then we push the items into the category list
                if (items.length != 0) {
                    categoryItems.push(items);
                    totalItems = categoryItems.flat().length;
                    console.log(`Category: ${categoryName} . Total Items currently: ${totalItems} Required: ${totalReq}`);
                       
                } else {
                    flag = false;
                    console.log("EMPTY");
                }
                
                    
            } catch (error) {
                // Handle error
                console.error(error);
            }
            
            
        }
        console.log(`Finished ${categoryName}`);

        // Push the categorylist to the final list of total items
        listItems.push(categoryItems.flat());

    }

    console.log("Main complete");
    // Organising the data we want to keep
    const newArray = listItems.flat().map((item)=> {
        return{
            // Only choosing to keep certain properties of each item in our list
            name: item.name,
            barcode: item.barcode,
            variety: item.variety,
            brand: item.brand,
            sku: item.sku,
            unit: item.unit,
            price: {
                originalPrice: item.price.originalPrice,
                salePrice: item.price.salePrice,
                savePrice: item.price.savePrice,
                isClubPrice: item.price.isClubPrice,
                isSpecial: item.price.isSpecial,
                isNew: item.price.isNew,
                discount: item.price.discount,
                total: item.price.total,
                purchasingUnitPrice: item.price.purchasingUnitPrice,
                orderedPrice: item.price.orderedPrice,

            },
            size: item.size


        }
    });
    // Filtering out the items that are repeats (i.e potentially ones that are from multiple categories)
    const uniqueItems = newArray.filter((item,index)=> newArray.indexOf(item)===index);

    fs.writeFileSync("./controller/CountdownScraper/countdown.json",JSON.stringify(uniqueItems,null,2));

}
main();

// const port = 4000; // Choose a port for your server
// app.listen(port, () => {
//   console.log(`Server is running on port ${port}`);
// });
